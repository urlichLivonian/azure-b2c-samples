...
namespace CarvedRock.UI.Services
{
    ...
    public class WishlistService : IWishlistService
    {
        ...
        {            
            ...
            var createReq = await CreateAuthenticatedRequest(HttpMethod.Post, $"{_baseAddress}/wishlist");
        ...
        private async Task<HttpRequestMessage> CreateAuthenticatedRequest(HttpMethod method, string requestUrl)
        {
            try
            {
                ...
                var accessToken = await _tokenAcquisition.GetAccessTokenForUserAsync(new[] { _readScope, _writeScope });
